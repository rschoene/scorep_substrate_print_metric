...
static void print_metrics_set_callbacks(
...
static void print_metrics_init_mpp(void)
{
...
static void print_metrics_new_definition_handle(SCOREP_AnyHandle handle,
		SCOREP_HandleType type)
{
	...
	case SCOREP_HANDLE_TYPE_SAMPLING_SET:
	{
		...
		for (uint8_t met_nr = 0; met_nr < nr_metric_handles; met_nr++)
		{
			...
			assert(
					callbacks->SCOREP_SamplingSetHandle_GetMetricOccurrence(
							handle) == SCOREP_METRIC_OCCURRENCE_ASYNCHRONOUS);
			...
			metrics[i].value_type = callbacks->SCOREP_MetricHandle_GetValueType(
					metric_handle);
		}
		break;
	}
...
static void print_metrics_write_metric(struct SCOREP_Location* location,
		uint64_t timestamp, SCOREP_SamplingSetHandle samplingSet,
...
/* called after unify */
static void print_metrics_write_data()
{
...
static void print_metrics_finalize()
{
	for (size_t i = 0; i < nr_metrics; i++)
	{
		free(metrics[i].name);
...
static void print_metrics_create_location(
...
static void print_metrics_init(size_t ignored)
{
	gettimeofday(&start, NULL);
}

static void print_metrics_unify()
{
